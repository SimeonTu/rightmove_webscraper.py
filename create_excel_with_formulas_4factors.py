#!/usr/bin/env python3
"""
Create Excel File with Dynamic Formulas (4 Factors)

This script converts the properties CSV to an Excel file with dynamic formulas
that automatically calculate scores based on price, distance, drive time, and size.
...
def add_formulas_to_excel(excel_file, price_col, distance_col, time_col, size_col, data_start_row, data_end_row):
    """
    Add dynamic formulas to the Excel file for calculating scores (4 factors).

    Args:
        excel_file: Path to Excel file
        price_col: Column letter for price (e.g., 'B')
        distance_col: Column letter for distance (e.g., 'X')
        time_col: Column letter for drive time (e.g., 'Y')
        size_col: Column letter for size (e.g., 'U')
    ...
    headers = {
        ...
        score_start_col + 2: 'time_score',
        score_start_col + 3: 'size_score',
        score_start_col + 4: 'combined_score_4factors'
    }
    ...
    time_score_col = get_column_letter(score_start_col + 2)
    size_score_col = get_column_letter(score_start_col + 3)
    combined_score_col = get_column_letter(score_start_col + 4)
    ...
    for row in range(data_start_row, data_end_row + 1):
        ...
        # Drive Time Score Formula: Normalize time (shorter is better)
        time_score_formula = (
            f'=IF(AND(ISNUMBER({time_cell}), '
            f'ISNUMBER(MIN({time_col}${data_start_row}:{time_col}${data_end_row})), '
            f'ISNUMBER(MAX({time_col}${data_start_row}:{time_col}${data_end_row}))), '
            f'IF(MAX({time_col}${data_start_row}:{time_col}${data_end_row})=MIN({time_col}${data_start_row}:{time_col}${data_end_row}), 50, '
            f'ROUND((MAX({time_col}${data_start_row}:{time_col}${data_end_row})-{time_cell})/'
            f'(MAX({time_col}${data_start_row}:{time_col}${data_end_row})-MIN({time_col}${data_start_row}:{time_col}${data_end_row}))*100, 2)), "")'
        )
        ...
        # Combined Score Formula: Average of four scores (equal weighting)
        combined_score_formula = (
            f'=IF(AND(ISNUMBER({price_score_col}{row}), ISNUMBER({distance_score_col}{row}), '
            f'ISNUMBER({time_score_col}{row}), ISNUMBER({size_score_col}{row})), '
            f'ROUND(({price_score_col}{row}+{distance_score_col}{row}+{time_score_col}{row}+{size_score_col}{row})/4, 2), "")'
        )

        # Add formulas to cells
        ws[f"{price_score_col}{row}"] = price_score_formula
        ws[f"{distance_score_col}{row}"] = distance_score_formula
        ws[f"{time_score_col}{row}"] = time_score_formula
    ...
    ws.column_dimensions[time_score_col].width = 12
    ws.column_dimensions[size_score_col].width = 12
    ws.column_dimensions[combined_score_col].width = 20
...
def main():
    ...
    output_file = "/Users/simeontu/Documents/vscode-projects/rightmove_webscraper.py/results/enhanced_scrape_2025-Oct-16_at_17h42m/enhanced_properties_dynamic_4factors.xlsx"

    logger.info("=" * 80)
    logger.info("CREATE EXCEL FILE WITH DYNAMIC FORMULAS (4 FACTORS)")
    logger.info("=" * 80)
    ...
    time_col_idx = df.columns.get_loc('drive_time_to_chinatown_minutes') + 1
    ...
    time_col = get_column_letter(time_col_idx)
    ...
    logger.info(f"  Drive Time: Column {time_col}")
    ...
    logger.info(f"  • Update combined_score based on price, distance, drive time, and size")
    logger.info(f"  • Use equal weighting (25% each)")
