#!/usr/bin/env python3
"""
Property Analysis and Scoring Script - Limited to 30 properties for testing
...
def main():
    ...
    output_file = "/Users/simeontu/Documents/vscode-projects/rightmove_webscraper.py/results/enhanced_scrape_2025-Oct-15_at_18h49m/enhanced_properties_with_scores_30.csv"
    ...
    # Filter to only properties with size data and limit to 30 for testing
    df_with_size = df.dropna(subset=['property_size_sqm']).head(30)
    logger.info(f"Properties with size data: {len(df_with_size)} out of {len(df)}")
    logger.info(f"Processing only first 30 properties with size data for testing...")
    ...
    for idx, row in df_with_size.iterrows():
        logger.info(f"Processing property {idx + 1}/{len(df_with_size)}: {row['address'][:50]}...")
        ...
        logger.info(f"  -> Price: £{row['price']:.0f} | Size: {row['property_size_sqm']:.0f} sqm | Distance: {distance_str} km | Score: {overall_score:.1f}")
    ...
    for idx, row in top_10.iterrows():
        logger.info(f"{idx+1}. Score: {row['overall_score']:.1f} | Price: £{row['price']:.0f} | Size: {row['property_size_sqm']:.0f} sqm | Distance: {row['distance_to_center_km']:.1f} km | {row['address']}")
